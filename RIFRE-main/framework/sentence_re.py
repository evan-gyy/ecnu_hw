...
class Sentence_RE(nn.Module):
    ...
    def train_once(self, loader, warmup=True):
        ...
        for iter, data in enumerate(t):
            ...
            # loss
            loss, acc = self.acc_loss(logits, sent_label, 6)
            # Log
    ...
    def eval_semeval(self, eval_loader, mode='dev'):
        self.model.eval()
    ...
    def acc_loss(self, logits, sent_label, nums=6):
    ...
    def save_pred_label(self, test_path):
        labels = self.pred_labels
        with open(test_path, 'r') as f:
            test_data = [json.loads(line) for line in f.read().split('\n') if line]
        with open(test_path.replace(".txt", "_result.txt"), 'w') as f:
            current = ''
            for i, sent in enumerate(test_data):
                if current != sent['file_name'] and current != '':
                    f.write('\n')
                block = ' '.join([str(labels[i])] + sent['pos'])
                f.write(block + '\t')
                current = sent['file_name']
